...
const BACKTEST_INPUTS_WIDTH_KEY = 'st_backtesting_inputs_panel_width_v1'

function clampInputsPanelWidth(w: number): number {
  return Math.min(720, Math.max(320, Math.round(w)))
}
...
function formatYmdHmsAmPm(value: unknown, displayTimeZone: 'LOCAL' | string): string {
  ...
  const d = parseBackendDate(isNaive ? raw.replace(' ', 'T') + '+05:30' : value)
  if (!d) return ''
...
function downsampleKeep<T>(
  ...
  return values.filter(
    (v, i) => keep?.(v) === true || i % step === 0 || i === values.length - 1,
  )
}
...
export function BacktestingPage() {
  ...
  const drawerEnabled =
    tab === 'STRATEGY' || tab === 'PORTFOLIO' || tab === 'PORTFOLIO_STRATEGY'
  ...
  const [inputsPanelWidth, setInputsPanelWidth] = useState<number>(() => {
    try {
      const raw = window.localStorage.getItem(BACKTEST_INPUTS_WIDTH_KEY)
      const n = raw ? Number(raw) : Number.NaN
      return Number.isFinite(n) ? clampInputsPanelWidth(n) : 420
    } catch {
      return 420
    }
  })
  const [inputsPanelResizing, setInputsPanelResizing] = useState(false)
  const inputsPanelResizeRef = useRef<{ startX: number; startWidth: number } | null>(
    null,
  )

  useEffect(() => {
    try {
      window.localStorage.setItem(BACKTEST_INPUTS_WIDTH_KEY, String(inputsPanelWidth))
    } catch {
  ...
  useEffect(() => {
    if (!inputsPanelResizing) return

    const prevCursor = document.body.style.cursor
    document.body.style.cursor = 'col-resize'

    const onMove = (e: PointerEvent) => {
      const ctx = inputsPanelResizeRef.current
      if (!ctx) return
      const next = ctx.startWidth + (e.clientX - ctx.startX)
      setInputsPanelWidth(clampInputsPanelWidth(next))
    }

    const onUp = () => {
      inputsPanelResizeRef.current = null
      setInputsPanelResizing(false)
    }
  ...
  const [portfolioMethod, setPortfolioMethod] =
    useState<PortfolioMethod>('TARGET_WEIGHTS')
  ...
  const [portfolioChargesModel, setPortfolioChargesModel] =
    useState<ChargesModel>('BROKER')
  const [portfolioChargesBroker, setPortfolioChargesBroker] =
  ...
  const [executionFillTiming, setExecutionFillTiming] =
    useState<FillTiming>('NEXT_OPEN')
  ...
  const [executionChargesModel, setExecutionChargesModel] =
    useState<ChargesModel>('BROKER')
  const [executionChargesBroker, setExecutionChargesBroker] =
    useState<BrokerName>('zerodha')
  ...
  const [strategyDslTab, setStrategyDslTab] = useState<'ENTRY' | 'EXIT'>('ENTRY')
  ...
  const [strategyChargesModel, setStrategyChargesModel] =
    useState<ChargesModel>('BROKER')
  ...
  const [portfolioStrategyEntryDsl, setPortfolioStrategyEntryDsl] =
    useState('RSI(14) < 30')
  const [portfolioStrategyExitDsl, setPortfolioStrategyExitDsl] =
    useState('RSI(14) > 70')
  ...
  const [portfolioStrategyInitialCash, setPortfolioStrategyInitialCash] =
    useState(100000)
  const [portfolioStrategyMaxOpenPositions, setPortfolioStrategyMaxOpenPositions] =
    useState(10)
  ...
  const [portfolioStrategyRankingWindow, setPortfolioStrategyRankingWindow] =
    useState(20)
  const [portfolioStrategySizingMode, setPortfolioStrategySizingMode] =
    useState<PortfolioStrategySizingMode>('CASH_PER_SLOT')
  const [portfolioStrategyPositionSizePct, setPortfolioStrategyPositionSizePct] =
    useState(10)
  const [portfolioStrategyFixedCashPerTrade, setPortfolioStrategyFixedCashPerTrade] =
    useState(10000)
  const [portfolioStrategyMinHoldingBars, setPortfolioStrategyMinHoldingBars] =
    useState(0)
  const [portfolioStrategyCooldownBars, setPortfolioStrategyCooldownBars] = useState(0)
  const [portfolioStrategyMaxSymbolAllocPct, setPortfolioStrategyMaxSymbolAllocPct] =
    useState(0)
  const [portfolioStrategyStopLossPct, setPortfolioStrategyStopLossPct] = useState(0)
  const [portfolioStrategyTakeProfitPct, setPortfolioStrategyTakeProfitPct] =
    useState(0)
  const [portfolioStrategyTrailingStopPct, setPortfolioStrategyTrailingStopPct] =
    useState(0)
  const [
    portfolioStrategyMaxEquityDdGlobalPct,
    setPortfolioStrategyMaxEquityDdGlobalPct,
  ] = useState(0)
  const [
    portfolioStrategyMaxEquityDdTradePct,
    setPortfolioStrategyMaxEquityDdTradePct,
  ] = useState(0)
  ...
  const [pinnedRunIdsByTab, setPinnedRunIdsByTab] = useState<
    Record<DrawerBacktestTab, number[]>
  >({
    STRATEGY: [],
    PORTFOLIO: [],
    PORTFOLIO_STRATEGY: [],
  ...
  const [pinnedRunErrorsById, setPinnedRunErrorsById] = useState<
  ...
  const pinnedRunIds = useMemo(() => {
    ...
    pinnedRunIdsByTab.PORTFOLIO,
    pinnedRunIdsByTab.PORTFOLIO_STRATEGY,
    pinnedRunIdsByTab.STRATEGY,
    tab,
  ])
  ...
  const pinRun = useCallback(
    (runId: number) => {
      if (tab !== 'STRATEGY' && tab !== 'PORTFOLIO' && tab !== 'PORTFOLIO_STRATEGY')
        return
  ...
  const closePinnedRun = useCallback(
    (runId: number) => {
      if (tab !== 'STRATEGY' && tab !== 'PORTFOLIO' && tab !== 'PORTFOLIO_STRATEGY')
        return
      setPinnedRunIdsByTab((prev) => ({
        ...prev,
        [tab]: prev[tab].filter((x) => x !== runId),
      }))
  ...
  const renderDrawerDetails = useCallback(
    (run: BacktestRun) => {
      ...
      if (tab === 'PORTFOLIO_STRATEGY') {
        return (
          <PortfolioStrategyRunDetailsCard
            run={run}
    ...
    (run: BacktestRun): DeploymentPrefill | null => {
      ...
      const baseCfg: Record<string, unknown> = {
        ...
        execution_target: String(
          (cfg as any).execution_target ?? 'PAPER',
        ).toUpperCase(),
  ...
  const buildDeploymentNameForRun = useCallback(
    (run: BacktestRun, prefill: DeploymentPrefill): string => {
      const u = getRunUniverse(run)
      const broker = String(u.broker_name ?? 'zerodha').toLowerCase()
      const brokerPart = slugifyNamePart(
        broker === 'angelone' ? 'angelone' : 'zerodha',
        16,
      )
  ...
  const renderDetails = useCallback(
    (run: BacktestRun): string => {
      ...
      if (run.kind === 'PORTFOLIO') {
        ...
        const budget =
          cfg.budget_pct != null ? `${Number(cfg.budget_pct).toFixed(0)}%` : '—'
      ...
      if (run.kind === 'STRATEGY') {
        ...
        if (Number.isFinite(ddGlobal) && ddGlobal > 0)
          ddParts.push(`eqDD global ${ddGlobal}%`)
        if (Number.isFinite(ddTrade) && ddTrade > 0)
          ddParts.push(`eqDD trade ${ddTrade}%`)
      ...
      if (run.kind === 'PORTFOLIO_STRATEGY') {
        ...
        const shorten = (s: string) => (s.length > 42 ? `${s.slice(0, 39)}…` : s)
        const posTxt =
          Number.isFinite(maxPos) && maxPos > 0 ? ` • maxPos ${maxPos}` : ''
      ...
      if (run.kind === 'EXECUTION') {
        ...
        const fill = String(cfg.fill_timing ?? '—')
        const slip =
          cfg.slippage_bps != null ? `${Number(cfg.slippage_bps).toFixed(0)}bps` : '—'
        const chargesModel = String(cfg.charges_model ?? 'BPS')
        const dpTxt = cfg.include_dp_charges === false ? 'no-DP' : 'DP'
        const charges =
          chargesModel === 'BROKER'
  ...
  const applyRunToInputs = useCallback(
    (run: BacktestRun) => {
      ...
      if (run.kind === 'PORTFOLIO') {
        ...
        setPortfolioChargesBroker(
          String(cfg.charges_broker ?? 'zerodha') === 'angelone'
            ? 'angelone'
            : 'zerodha',
        )
        ...
        setPortfolioGateSymbolExchange(
          String(cfg.gate_symbol_exchange ?? 'NSE') || 'NSE',
        )
        setPortfolioGateGroupId(
          typeof cfg.gate_group_id === 'number' ? (cfg.gate_group_id as number) : '',
        )
        ...
        setRotationWindow(
          Number(cfg.ranking_window ?? rotationWindow) || rotationWindow,
        )
      ...
      if (run.kind === 'STRATEGY') {
        ...
        const first = symbols[0] ?? null
        if (first) {
          const ex = String(first.exchange ?? 'NSE')
            .trim()
            .toUpperCase()
      ...
      if (run.kind === 'PORTFOLIO_STRATEGY') {
        ...
        setPortfolioStrategyEntryDsl(
          String(cfg.entry_dsl ?? '').trim() || 'RSI(14) < 30',
        )
        ...
        setPortfolioStrategyRankingMetric(
          rMetric === 'PERF_PCT' ? 'PERF_PCT' : 'PERF_PCT',
        )
        ...
        setPortfolioStrategyFixedCashPerTrade(
          Number(cfg.fixed_cash_per_trade ?? 10000) || 0,
        )
        ...
        setPortfolioStrategyMaxSymbolAllocPct(
          Number(cfg.max_symbol_alloc_pct ?? 0) || 0,
        )
        ...
        setPortfolioStrategyMaxEquityDdGlobalPct(
          Number(cfg.max_equity_dd_global_pct ?? 0) || 0,
        )
        setPortfolioStrategyMaxEquityDdTradePct(
          Number(cfg.max_equity_dd_trade_pct ?? 0) || 0,
        )
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      ...
      if (
        compareRunId !== '' &&
        typeof compareRunId === 'number' &&
        deleted.includes(compareRunId)
      ) {
        setCompareRunId('')
      }
      await refreshRuns()
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (kind === 'STRATEGY') {
        if (!strategySymbolKey)
          throw new Error('Please select a symbol for Strategy backtests.')
        if (!strategyEntryDsl.trim() || !strategyExitDsl.trim()) {
          throw new Error(
            'Both entry and exit DSL are required for Strategy backtests.',
          )
        }
        if (strategyProduct === 'CNC' && strategyDirection !== 'LONG') {
          throw new Error(
            'CNC does not allow short selling. Use direction LONG or switch to MIS.',
          )
        }
        if (strategyProduct === 'MIS' && strategyTimeframe === '1d') {
      ...
      if (kind === 'PORTFOLIO_STRATEGY') {
        if (universeMode !== 'GROUP') {
          throw new Error(
            'Portfolio strategy backtests currently require Universe = Group.',
          )
        }
        if (typeof groupId !== 'number') {
          throw new Error(
            'Please select a portfolio group for Portfolio strategy backtests.',
          )
        }
        if (!portfolioStrategyEntryDsl.trim() || !portfolioStrategyExitDsl.trim()) {
          throw new Error(
            'Both entry and exit DSL are required for Portfolio strategy backtests.',
          )
        }
        if (
            ...
            ? {
                ...
                gate_symbol_exchange:
                  portfolioGateSource === 'SYMBOL'
                    ? portfolioGateSymbolExchange
                    : 'NSE',
                gate_symbol:
                  portfolioGateSource === 'SYMBOL' ? portfolioGateSymbol : '',
                gate_group_id:
                  portfolioGateSource === 'GROUP_INDEX' &&
                  typeof portfolioGateGroupId === 'number'
    ...
    if (tab === 'STRATEGY' || tab === 'PORTFOLIO_STRATEGY') {
      cols.push({
        ...
        renderCell: (params) => {
          const run = params.row as BacktestRun
          const disabled =
            run.status !== 'COMPLETED' || running || deployingRunId === run.id
          return (
  ...
  const selectedUniverseSummary = useMemo(() => {
    ...
    if (universeMode === 'GROUP')
      return groupDetail ? `Group: ${groupDetail.name}` : 'Group: (select)'
  ...
  const signalPresets = useMemo(
    () => [
      {
        id: 'RSI_OVERSOLD',
        label: 'RSI oversold (RSI(14) < 30)',
        mode: 'DSL' as const,
        dsl: 'RSI(14) < 30',
      },
      {
        id: 'SMA_TREND',
        label: 'Uptrend (MA(50) > MA(200))',
        mode: 'DSL' as const,
        dsl: 'MA(50) > MA(200)',
      },
      {
      ...
      {
        id: 'TOP_MOM_20D',
        label: 'Top‑N momentum (20D, monthly)',
        mode: 'RANKING' as const,
      },
      {
        id: 'TOP_MOM_60D',
        label: 'Top‑N momentum (60D, weekly)',
        mode: 'RANKING' as const,
      },
    ],
    [],
  ...
  const signalSummaryColumns = useMemo((): GridColDef[] => {
    const infer = (r: Record<string, unknown>): string[] => {
        ...
        const edgeWord =
          avg > 0 && win >= 50
            ...
            : avg < 0 && win < 50
              ...
              : 'Edge: mixed'
        bullets.push(
          `${edgeWord} (win ${win.toFixed(1)}%, avg ${fmtPct(avg, 2)}, n=${count})`,
        )
      ...
      if (Number.isFinite(p50) && Number.isFinite(p90) && Number.isFinite(windowDays)) {
        ...
        bullets.push(
          `Typical: median ${fmtPct(p50, 2)} over ${windowDays} trading days`,
        )
      }
      ...
      if (Number.isFinite(p10) && Number.isFinite(windowDays)) {
        ...
        bullets.push(
          `${riskWord}: worst 10% ≤ ${fmtPct(p10, 2)} over ${windowDays} trading days`,
        )
      }
  ...
  const executionEquityCandles = useMemo((): PriceCandle[] => {
    ...
    const realistic =
      (result.realistic as Record<string, unknown> | undefined) ?? undefined
    const series =
      (realistic?.series as Record<string, unknown> | undefined) ?? undefined
    if (!series) return []
    ...
    >
      ...
      >
        <Box>
          ...
          <Tabs
            value={tab}
            onChange={(_e, v) => setTab(v as BacktestTab)}
            sx={{ mt: 0 }}
          >
      ...
      >
        <Box
          sx={{
            width: { xs: '100%', lg: `${inputsPanelWidth}px` },
            minWidth: { lg: 320 },
            maxWidth: { lg: 720 },
            flexShrink: 0,
          }}
            ...
            <Stack spacing={2}>
                ...
                <Stack spacing={1.5}>
                  <FormControl fullWidth size="small">
                    ...
                    >
                      ...
                      {executionBaseRuns
                        .filter(
                          (r) => r.kind === 'PORTFOLIO' && r.status === 'COMPLETED',
                        )
                        .map((r) => {
                          const groupId = Number(
                            (r.config as any)?.universe?.group_id ?? NaN,
                          )
                          const groupName =
                    ...
                    >
                      <Chip
                        ...
                        label={`Base #${executionBaseRun.id} • ${String(
                          (executionBaseRun.config as any)?.config?.method ??
                            'PORTFOLIO',
                        )}`}
                      />
                      <Chip
                  ...
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setExecutionFillTiming(e.target.value as FillTiming)
                        }
                      >
                  ...
                  <Stack spacing={1}>
                    <Stack direction="row" spacing={1}>
                      ...
                      <FormControl fullWidth size="small">
                        <InputLabel id="bt-exec-charges-model-label">
                          Charges
                        </InputLabel>
                        <Select
                          ...
                          onChange={(e) =>
                            setExecutionChargesModel(e.target.value as ChargesModel)
                          }
                        >
                          <MenuItem value="BROKER">
                            Broker estimate (India equity)
                          </MenuItem>
                    ...
                    <Stack direction="row" spacing={1}>
                      {executionChargesModel === 'BPS' ? (
                        <TextField
                          ...
                          onChange={(e) =>
                            setExecutionChargesBps(Number(e.target.value))
                          }
                          inputProps={{ min: 0, max: 2000 }}
                          fullWidth
                        />
                        ...
                        <FormControl fullWidth size="small">
                          <InputLabel id="bt-exec-charges-broker-label">
                            Broker
                          </InputLabel>
                          <Select
                            ...
                            onChange={(e) =>
                              setExecutionChargesBroker(e.target.value as BrokerName)
                            }
                          >
                            <MenuItem value="zerodha">Zerodha</MenuItem>
                            <MenuItem value="angelone">AngelOne</MenuItem>
                        ...
                        executionProduct === 'CNC' && (
                          <FormControl fullWidth size="small">
                            ...
                            <Select
                              ...
                              onChange={(e) =>
                                setExecutionIncludeDpCharges(e.target.value === 'ON')
                              }
                            >
                    ...
                    {executionChargesModel === 'BROKER' && (
                      <Typography variant="caption" color="text.secondary">
                        Estimates India equity charges (brokerage + STT + exchange +
                        SEBI + stamp duty (WB buy-side) + GST and optional DP on
                    ...
                    <Stack direction="row" spacing={1}>
                      ...
                      <FormControl fullWidth size="small">
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setRankingCadence(e.target.value as RankingCadence)
                          }
                        >
                ...
                <Stack spacing={1.5}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setPortfolioMethod(e.target.value as PortfolioMethod)
                      }
                    >
                  ...
                  <Paper variant="outlined" sx={{ p: 1 }}>
                    <Stack spacing={1}>
                      <Typography variant="caption" color="text.secondary">
                        Gate / regime filter (optional): skip rebalances unless the
                        condition is true.
                      </Typography>
                      <Stack direction="row" spacing={1}>
                        <FormControl fullWidth size="small">
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setPortfolioGateSource(e.target.value as GateSource)
                            }
                          >
                            <MenuItem value="NONE">(none)</MenuItem>
                            <MenuItem value="GROUP_INDEX">
                              Group index (synthetic)
                            </MenuItem>
                        ...
                        {portfolioGateSource === 'GROUP_INDEX' && (
                          <TextField
                            ...
                            onChange={(e) =>
                              setPortfolioGateMinCoveragePct(Number(e.target.value))
                            }
                            inputProps={{ min: 0, max: 100 }}
                            sx={{ width: 160 }}
                          />
                      ...
                      {portfolioGateSource !== 'NONE' && (
                        <>
                          ...
                          {portfolioGateSource === 'GROUP_INDEX' && (
                            <FormControl fullWidth size="small">
                              <InputLabel id="bt-pf-gate-group-label">
                                Gate group
                              </InputLabel>
                              <Select
                          ...
                          {portfolioGateSource === 'SYMBOL' && (
                            <Stack direction="row" spacing={1}>
                              <FormControl size="small" sx={{ minWidth: 120 }}>
                                <InputLabel id="bt-pf-gate-exch-label">
                                  Exchange
                                </InputLabel>
                                <Select
                              ...
                              <TextField
                                ...
                                onChange={(e) =>
                                  setPortfolioGateSymbol(e.target.value.toUpperCase())
                                }
                          ...
                          <Typography variant="caption" color="text.secondary">
                            Coverage applies only to Group index gates (dynamic
                            availability set; default 90%).
                          </Typography>
                        </>
                      )}
                    </Stack>
                  ...
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setPortfolioCadence(e.target.value as PortfolioCadence)
                        }
                      >
                        <MenuItem value="WEEKLY">Weekly</MenuItem>
                        <MenuItem value="MONTHLY">Monthly</MenuItem>
                  ...
                  <Stack direction="row" spacing={1}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioMinTradeValue(Number(e.target.value))
                      }
                      inputProps={{ min: 0 }}
                      fullWidth
                    />
                    ...
                    <Stack direction="row" spacing={1}>
                      <FormControl fullWidth size="small">
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setPortfolioFillTiming(e.target.value as FillTiming)
                          }
                        >
                    ...
                    <Stack direction="row" spacing={1}>
                      <FormControl fullWidth size="small">
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setPortfolioChargesModel(e.target.value as ChargesModel)
                          }
                        >
                          <MenuItem value="BROKER">
                            Broker estimate (India equity)
                      ...
                      {portfolioChargesModel === 'BPS' ? (
                        <TextField
                          ...
                          onChange={(e) =>
                            setPortfolioChargesBps(Number(e.target.value))
                          }
                          inputProps={{ min: 0, max: 2000 }}
                          fullWidth
                        />
                        ...
                        <FormControl fullWidth size="small">
                          <InputLabel id="bt-pf-charges-broker-label">
                            Broker
                          </InputLabel>
                          <Select
                            ...
                            onChange={(e) =>
                              setPortfolioChargesBroker(e.target.value as BrokerName)
                            }
                          >
                            <MenuItem value="zerodha">Zerodha</MenuItem>
                            <MenuItem value="angelone">AngelOne</MenuItem>
                      ...
                      portfolioProduct === 'CNC' && (
                        <FormControl fullWidth size="small">
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setPortfolioIncludeDpCharges(e.target.value === 'ON')
                            }
                          >
                            <MenuItem value="ON">Include DP (delivery sell)</MenuItem>
                            <MenuItem value="OFF">Exclude DP</MenuItem>
              ...
              {tab === 'PORTFOLIO_STRATEGY' && (
                <Stack spacing={1.5}>
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setPortfolioStrategyTimeframe(
                            e.target.value as StrategyTimeframe,
                          )
                        }
                      >
                        ...
                        <MenuItem
                          value="1d"
                          disabled={portfolioStrategyProduct === 'MIS'}
                        >
                          1d
                        </MenuItem>
                      </Select>
                    ...
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setPortfolioStrategyProduct(e.target.value as ProductType)
                        }
                      >
                        <MenuItem value="CNC">CNC (delivery)</MenuItem>
                        <MenuItem value="MIS">MIS (intraday)</MenuItem>
                    ...
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setPortfolioStrategyDirection(
                            e.target.value as StrategyDirection,
                          )
                        }
                  ...
                  <Box>
                    <Tabs
                      value={portfolioStrategyDslTab}
                      onChange={(_e, v) =>
                        setPortfolioStrategyDslTab(v === 'EXIT' ? 'EXIT' : 'ENTRY')
                      }
                      variant="fullWidth"
                    >
                      <Tab value="ENTRY" label="Entry DSL" />
                  ...
                  <Stack direction="row" spacing={1}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioStrategyInitialCash(Number(e.target.value))
                      }
                      inputProps={{ min: 0 }}
                      fullWidth
                    />
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioStrategyRankingWindow(Number(e.target.value))
                      }
                      inputProps={{ min: 1, max: 1000 }}
                      fullWidth
                      disabled={portfolioStrategyAllocationMode !== 'RANKING'}
                  ...
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      ...
                      >
                        <MenuItem value="CASH_PER_SLOT">
                          Use cash per slot (max positions)
                        </MenuItem>
                  ...
                  <Stack direction="row" spacing={1}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioStrategyStopLossPct(Number(e.target.value))
                      }
                      inputProps={{ min: 0, max: 100 }}
                      fullWidth
                    />
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioStrategyTakeProfitPct(Number(e.target.value))
                      }
                      inputProps={{ min: 0, max: 100 }}
                      fullWidth
                    />
                    <TextField
                      ...
                      onChange={(e) =>
                        setPortfolioStrategyTrailingStopPct(Number(e.target.value))
                      }
                      inputProps={{ min: 0, max: 100 }}
                      fullWidth
                    />
              ...
              {tab === 'STRATEGY' && (
                <Stack spacing={1.5}>
                  ...
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      <InputLabel id="bt-strategy-timeframe-label">
                        Timeframe
                      </InputLabel>
                      <Select
                        ...
                        onChange={(e) =>
                          setStrategyTimeframe(e.target.value as StrategyTimeframe)
                        }
                      >
                        <MenuItem value="1m">1m</MenuItem>
                        <MenuItem value="5m">5m</MenuItem>
                  ...
                  <Stack direction="row" spacing={1}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setStrategyProduct(e.target.value as ProductType)
                        }
                      >
                        <MenuItem value="CNC">CNC (delivery)</MenuItem>
                        <MenuItem value="MIS">MIS (intraday)</MenuItem>
                    ...
                    <FormControl fullWidth size="small">
                      <InputLabel id="bt-strategy-direction-label">
                        Direction
                      </InputLabel>
                      <Select
                        ...
                        onChange={(e) =>
                          setStrategyDirection(e.target.value as StrategyDirection)
                        }
                  ...
                  <Box>
                    <Tabs
                      value={strategyDslTab}
                      onChange={(_e, v) =>
                        setStrategyDslTab(v === 'EXIT' ? 'EXIT' : 'ENTRY')
                      }
                      variant="fullWidth"
                    >
                      <Tab value="ENTRY" label="Entry DSL" />
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    <TextField
                      ...
                      onChange={(e) =>
                        setStrategyPositionSizePct(Number(e.target.value))
                      }
                      inputProps={{ min: 0, max: 100 }}
                      fullWidth
                    />
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    <TextField
                      ...
                      onChange={(e) =>
                        setStrategyTrailingStopPct(Number(e.target.value))
                      }
                      inputProps={{ min: 0, max: 100 }}
                      fullWidth
                    />
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap">
                {(tab === 'STRATEGY' || tab === 'PORTFOLIO_STRATEGY') && (
                  <Button
                    size="small"
                    variant="outlined"
        ...
        {lgUp && (
          <Box
            onPointerDown={(e) => {
              if (inputsPanelResizing) return
              e.preventDefault()
              inputsPanelResizeRef.current = {
                startX: e.clientX,
                startWidth: inputsPanelWidth,
              }
              setInputsPanelResizing(true)
            }}
            sx={{
              width: 8,
              alignSelf: 'stretch',
              cursor: 'col-resize',
              borderRadius: 1,
              bgcolor: inputsPanelResizing ? 'action.hover' : 'transparent',
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Paper sx={{ p: 2 }}>
            <Stack spacing={2}>
              ...
              <Box
                sx={{
                  height: drawerEnabled ? 'calc(100vh - 320px)' : 260,
                  minHeight: 260,
                }}
              >
                <DataGrid
                  ...
                  {selectedRun ? (
                    ...
                    >
                      <Typography variant="body2" color="text.secondary">
                        Run #{selectedRun.id} • {selectedRun.kind} •{' '}
                        {selectedRun.status}
                      </Typography>
                      ...
                      {tab === 'SIGNAL' &&
                        ...
                        signalSummaryRows.length > 0 && (
                          <Box sx={{ mt: 1 }}>
                            <Typography variant="subtitle2">
                              Signal summary (by forward window)
                            </Typography>
                      ...
                      {tab === 'EXECUTION' &&
                        selectedRun.status === 'COMPLETED' &&
                        executionEquityCandles.length > 0 && (
                          <Box sx={{ mt: 1 }}>
                            <Typography variant="subtitle2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            void (async () => {
                              ...
                              setRunning(true)
                              try {
                                const universe = (config.universe ?? {}) as Record<
                                  string,
                                  unknown
                                >
                                const run = await createBacktestRun({
                                  ...
                                  universe: {
                                    ...
                                    broker_name:
                                      universe.broker_name === 'angelone'
                                        ? 'angelone'
                                        : 'zerodha',
                                    group_id:
                                      typeof universe.group_id === 'number'
                                        ? universe.group_id
                                        : null,
                                    symbols: Array.isArray(universe.symbols)
                                      ? (
                                          universe.symbols as Array<
      ...
      >
        ...
        <DialogContent sx={{ pt: 2 }}>
          {tab === 'STRATEGY' ? (
            <Stack spacing={2}>
              <DividerBlock title="Equity drawdown" />
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  fullWidth
                  helperText="0 disables. Peak since start; triggers exit + blocks new entries."
                />
                <TextField
              ...
              <Stack direction="row" spacing={1}>
                ...
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setStrategyChargesModel(e.target.value as ChargesModel)
                    }
                  >
              ...
              {strategyChargesModel === 'BPS' ? (
                ...
                <Stack spacing={1}>
                  <FormControl fullWidth size="small" disabled>
                    ...
                    <Select
                      labelId="bt-strategy-broker-label"
                      label="Broker"
                      value="zerodha"
                    >
                      <MenuItem value="zerodha">Zerodha</MenuItem>
                  ...
                  {strategyProduct === 'CNC' && (
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setStrategyIncludeDpCharges(e.target.value === 'ON')
                        }
                      >
                        <MenuItem value="ON">Include DP (delivery sell)</MenuItem>
                        <MenuItem value="OFF">Exclude DP</MenuItem>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Evaluates entry/exit at close and fills at next open. CNC is
                    long-only. MIS positions are squared off at end of day.
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    Broker model estimates India equity charges (brokerage + STT +
                    exchange + SEBI + stamp duty (WB buy-side) + GST and optional DP on
                    delivery sell). Rates are approximate.
                  </Typography>
                </Stack>
              )}
            ...
            <Stack spacing={2}>
              <DividerBlock title="Constraints" />
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) =>
                    setPortfolioStrategyMinHoldingBars(Number(e.target.value))
                  }
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    setPortfolioStrategyCooldownBars(Number(e.target.value))
                  }
                  inputProps={{ min: 0, max: 1000 }}
                  fullWidth
                />
              ...
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) =>
                    setPortfolioStrategySlippageBps(Number(e.target.value))
                  }
                  inputProps={{ min: 0, max: 2000 }}
                  fullWidth
                />
              ...
              {portfolioStrategyChargesModel === 'BPS' ? (
                <TextField
                  ...
                  onChange={(e) =>
                    setPortfolioStrategyChargesBps(Number(e.target.value))
                  }
                  inputProps={{ min: 0, max: 2000 }}
                  fullWidth
                />
                ...
                <Stack spacing={1}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Evaluates signals at close and fills at next open. CNC is long-only.
                    MIS positions are squared off at end of day.
                  </Typography>
                </Stack>
              )}
            </Stack>
            ...
            <Typography variant="body2" color="text.secondary">
              No advanced settings for this backtest type.
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        >
          ...
          {drawerRun &&
          (drawerRun.kind === 'STRATEGY' || drawerRun.kind === 'PORTFOLIO_STRATEGY') ? (
            <Tooltip title="Create a deployment (STOPPED) from this run">
              <Button
                size="small"
                variant="contained"
        ...
        <Box sx={{ px: 1.25, pb: 2, overflow: 'auto' }}>
          {drawerTab === 'selected' ? (
            selectedRun ? (
              selectedRun.kind !== tab ? (
                <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                  Selected run is {selectedRun.kind}; switch to the{' '}
                  {selectedRun.kind.toLowerCase()} tab to view details.
                </Typography>
              ) : (
            ...
            pinnedRunsById[drawerTab].kind !== tab ? (
              <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                This pinned run is {pinnedRunsById[drawerTab].kind}; switch tabs to
                view.
              </Typography>
            ) : (
      ...
      >
        ...
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 2 }}>
            This strategy already has a deployment. Creating another deployment for the
            same strategy can lead to duplicate trades and potential losses.
          </Alert>
          <Typography variant="body2" sx={{ mb: 1 }}>
            Existing deployment: <b>{deployConfirm?.existingName ?? ''}</b> (#
            {deployConfirm?.existingId ?? ''})
          </Typography>
          <Typography variant="body2" color="text.secondary">
            If you still want to create a new deployment entry, click “Create new anyway
            (unsafe)”.
          </Typography>
        </DialogContent>
        <DialogActions>
...
function StrategyRunEquityCard({
  ...
  const tradeStats =
    (result?.trade_stats as Record<string, unknown> | undefined) ?? null
  ...
  const baselineOverlays = useMemo(() => {
    ...
    const addOverlay = (name: string, color: string, curve: unknown) => {
      ...
      const points = equityCandles.map((c) => ({
        ts: c.ts,
        value: byTs.get(c.ts) ?? null,
      }))
    ...
    addOverlay(
      'Buy & hold (start→end)',
      '#6b7280',
      (baselines.start_to_end as unknown) ?? null,
    )
    if ((baselines as any).first_entry_to_end) {
      addOverlay(
        'Buy & hold (first entry→end)',
        '#9ca3af',
        (baselines as any).first_entry_to_end,
      )
    }
  ...
  const tradeColumns = useMemo((): GridColDef[] => {
    return [
      ...
      { field: 'side', headerName: 'Side', width: 110 },
      {
        ...
        type: 'number',
        valueFormatter: (value) =>
          fmtPrice((value as { value?: unknown })?.value ?? value, 2),
      },
      {
        field: 'sell_price',
        headerName: 'Sell',
    ...
    <Box sx={{ mt: 1 }}>
      ...
      {metrics ? (
        <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
          <Chip
            size="small"
            label={`Total: ${Number(metrics.total_return_pct ?? 0).toFixed(2)}%`}
          />
          <Chip
            size="small"
            label={`CAGR: ${Number(metrics.cagr_pct ?? 0).toFixed(2)}%`}
          />
          <Chip
            size="small"
            label={`Max DD: ${Number(metrics.max_drawdown_pct ?? 0).toFixed(2)}%`}
          />
          <Chip
            size="small"
            label={`Turnover: ${Number(metrics.turnover_pct_total ?? 0).toFixed(1)}%`}
          />
          ...
          <Chip
            size="small"
            label={`Charges: ${fmtInr(Number(metrics.total_charges ?? 0), 0)}`}
          />
          ...
          {tradeStats ? (
            <>
              ...
              <Chip
                size="small"
                label={`Win: ${Number(tradeStats.win_rate_pct ?? 0).toFixed(1)}%`}
              />
    ...
    <Box sx={{ mt: 1 }}>
      ...
      {metrics ? (
        <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
          ...
          <Chip
            size="small"
            label={`Win: ${Number(metrics.win_rate_pct ?? 0).toFixed(1)}%`}
          />
          {typeof profitValue === 'number' ? (
            <Chip size="small" label={`Profit (net): ${fmtInr(profitValue, 0)}`} />
          ) : null}
          {meta ? (
            <>
              ...
              />
              {Array.isArray(meta.symbols_missing) &&
              (meta.symbols_missing as unknown[]).length ? (
                <Chip
                  size="small"
                  variant="outlined"
                  color="warning"
              ...
              <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    downloadCsv(
                      `portfolio_strategy_closed_trades_run_${run.id}.csv`,
                      rows,
                    )
                  }}
                >
...
function PortfolioRunDetailsCard({
  ...
  const actionsColumns = useMemo((): GridColDef[] => {
    const fmtPctLocal = (value: unknown) =>
    ...
    return [
      ...
      { field: 'trades', headerName: 'Trades', width: 90 },
      {
        ...
        width: 120,
        valueFormatter: (value) =>
          fmtPctLocal((value as { value?: unknown })?.value ?? value),
      },
      {
        field: 'budget_used',
    ...
    <Box sx={{ mt: 1 }}>
      ...
      {metrics ? (
        <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
          ...
          <Chip
            size="small"
            label={`Rebalances: ${Number(metrics.rebalance_count ?? 0)}`}
          />
          {Number(metrics.rebalance_skipped_count ?? 0) > 0 ? (
            ...
            >
              <Chip
                size="small"
                color="warning"
                label={`Skipped: ${Number(metrics.rebalance_skipped_count ?? 0)}`}
              />
            </Tooltip>
          ) : null}
        </Stack>
            ...
            >
              ...
              {runs
                ...
                .map((r) => (
                  <MenuItem key={r.id} value={String(r.id)}>
                    #{r.id} —{' '}
                    {String((r.config as any)?.config?.method ?? r.title ?? '')}
                  </MenuItem>
                ))}
            </Select>
          </FormControl>
