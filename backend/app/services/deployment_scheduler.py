    ...
    for dep in deps:
        ...
        if tf == "1d":
            daily = cfg.get("daily_via_intraday") or {
                ...
                "buy_window_start_hhmm": "15:25",
                "sell_window_start_hhmm": "09:15",
            }
            ...
            def hhmm_to_time(raw: str) -> time:
                hh, mm = raw.split(":")
                return time(hour=int(hh), minute=int(mm))

            proxy_dt_ist = datetime.combine(now_i.date(), hhmm_to_time(proxy_hhmm))
            if now_i >= (proxy_dt_ist + timedelta(seconds=tolerance_seconds)):
            ...
            sell_dt_ist = datetime.combine(now_i.date(), hhmm_to_time(sell_hhmm))
            if now_i >= (sell_dt_ist + timedelta(seconds=tolerance_seconds)):
            ...
            buy_dt_ist = datetime.combine(now_i.date(), hhmm_to_time(buy_hhmm))
            if now_i >= (buy_dt_ist + timedelta(seconds=tolerance_seconds)):
